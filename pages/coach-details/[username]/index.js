...
export default function CoachDetails({
  ...
  const renderCoachProfile = (coachData) => {
    const job = jobs.find(
      (job) => job._id == coachData.map((e) => e.coachData.job),
    )
    let specialty = ''
    if (job.specialty && job.specialty.type == 'sport') {
      specialty = sports.find(
        (sport) => sport._id == coachData.map((e) => e.coachData.specialty),
      )
    } else if (job.specialty && job.specialty.type == 'dance') {
      specialty = dances.find(
        (dance) => dance._id == coachData.map((e) => e.coachData.specialty),
      )
    }
    return (
      <InfoCoach
  ...
  useEffect(() => {
    setCoachData(
      coachesList.filter((coach) => coach.username === router.query.username),
    )
  }, [router.query.id])
        ...
        <div className="coach__coachdetails">
          <div className="coach__cordonneBlock">
            <div className="coach__coachdetails__contact">
              {coachData && renderCoachProfile(coachData)}
            </div>
            <div className="coach__coachdetails__information">
              {coachData ? <ContactCoach coachData={coachData} /> : ''}
            </div>
          </div>
                ...
                {coachData.map((coach) => {
                  return tab == 1 ? (
                    <div className="tabsinfo__title">
                      <CoachBox
                        coachData={coach}
                        iconExclamation="../icon/exclamation.png"
                      >
                            ...
                            coach.coachData.privateCourseData.level && (
                              <div className="coachBox__content">
                                <div className="coachBox__content__title">
                                  Niveaux:{' '}
                                  <span>
                                    {coach.coachData.privateCourseData.level.join(
                                      ' , ',
                                    )}
                                  </span>
                                </div>
