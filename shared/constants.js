...
export const FRONT_END_PLATFORM_URL = () => {
  const { publicRuntimeConfig } = getConfig()

  const { localStorage, location } = typeof window !== 'undefined' && window
  ...
  if (localStorage) {
    if (
      ...
      return `https://dev.isporit.com?accessToken=${localStorage.getItem(
        'token',
      )}`
    }
    return `${
      publicRuntimeConfig.LOGIN_REDIRECT_URL
